...
class NutritionService(private val turso: TursoService) {
    ...
    suspend fun addMeal(userId: String, req: MealEntryRequest) {
        ...
        turso.execute(
            "INSERT INTO meals (id, user_id, name, grams, kcal_per_100, proteins_per_100, carbs_per_100, fat_per_100, meal_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            listOf(id, userId, req.name, req.grams.toString(), req.kcalPer100.toString(),
                req.proteinsPer100.toString(), req.carbsPer100.toString(),
                req.fatPer100.toString(), req.mealType, now.toString())
        )
    }

    suspend fun getUserMeals(userId: String): List<List<String?>> {
        val result = turso.execute(
            "SELECT name, grams, kcal_per_100, proteins_per_100, carbs_per_100, fat_per_100, meal_type, id FROM meals WHERE user_id = ? ORDER BY created_at ASC",
            listOf(userId)
        )
